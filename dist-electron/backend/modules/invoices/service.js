...
exports.createInvoicesService = void 0;
const kysely_1 = require("kysely");
const buildTicketCode = (orderNumber) => `TK-${orderNumber}`;
const buildWhatsappMessage = (invoice) => {
    ...
    const itemsText = invoice.items
        ...
        return [
            ...
            `   Cant: ${item.quantity} | Unit: ${formatMoney(item.unitPrice)} | Total: ${formatMoney(item.total)}`,
...
const createInvoicesService = (db) => {
    const list = async () => {
        ...
        const rows = await db
            ...
            'o.balance_due',
            'o.order_number',
            (0, kysely_1.sql) `c.first_name`.as('first_name'),
            (0, kysely_1.sql) `c.last_name`.as('last_name'),
            (0, kysely_1.sql) `c.phone`.as('client_phone')
        ])
            .orderBy('i.id desc')
            .execute();
...
exports.createInvoicesService = createInvoicesService;
