...
const service_js_1 = require("../payments/service.js");
const service_js_2 = require("../invoices/service.js");
const service_js_3 = require("../deliveries/service.js");
const orderItemSchema = zod_1.z.object({
    garmentTypeId: zod_1.z.number().nullable(),
    serviceId: zod_1.z.number().nullable(),
    description: zod_1.z.string().trim().min(3),
    quantity: zod_1.z.number().positive(),
    color: zod_1.z.string().nullable(),
    brand: zod_1.z.string().nullable(),
    sizeReference: zod_1.z.string().nullable(),
    material: zod_1.z.string().nullable(),
    receivedCondition: zod_1.z.string().nullable(),
    workDetail: zod_1.z.string().nullable(),
    stains: zod_1.z.string().nullable(),
    damages: zod_1.z.string().nullable(),
    missingAccessories: zod_1.z.string().nullable(),
    customerObservations: zod_1.z.string().nullable(),
    internalObservations: zod_1.z.string().nullable(),
    unitPrice: zod_1.z.number().nonnegative(),
    discountAmount: zod_1.z.number().nonnegative(),
    surchargeAmount: zod_1.z.number().nonnegative(),
...
const orderSchema = zod_1.z.object({
    ...
    discountTotal: zod_1.z.number().nonnegative(),
    paidAmount: zod_1.z.number().nonnegative(),
    initialPaymentMethodId: zod_1.z.number().nullable().optional(),
    initialPaymentReference: zod_1.z.string().nullable().optional(),
...
const createOrdersService = (db) => {
    const repository = (0, order_repository_js_1.createOrderRepository)(db);
    const paymentsService = (0, service_js_1.createPaymentsService)(db);
    const invoicesService = (0, service_js_2.createInvoicesService)(db);
    const deliveriesService = (0, service_js_3.createDeliveriesService)(db);
    ...
    const create = async (input) => {
        ...
        if (paidTotal > 0) {
            await paymentsService.create({
                orderId,
                paymentMethodId: parsed.initialPaymentMethodId,
                amount: paidTotal,
    ...
    const update = async (orderId, input) => {
        const parsed = orderSchema.parse(input);
    ...
    return {
        async dashboard() {
            const [clients, openOrders, dailySales, pendingBalance, openWarranties, dailyExpenses, recentOrders, paymentBreakdown] = await Promise.all([
                db
                    .selectFrom('clients')
                    .select((eb) => eb.fn.count('id').as('count'))
                    .executeTakeFirstOrThrow(),
                ...
                db
                    .selectFrom('orders')
                    .select((eb) => eb.fn.sum('balance_due').as('sum'))
                    .executeTakeFirst(),
                db
                    .selectFrom('warranties')
                    .select((eb) => eb.fn.count('id').as('count'))
