...
const createCashService = (db) => ({
    ...
    async close(input) {
        ...
        const totalsByMethod = await db
            ...
            .groupBy('pm.name')
            .execute();
        const paymentsTotal = totalsByMethod.reduce((sum, item) => sum + Number(item.amount ?? 0), 0);
        const deliveredOrders = await db
            ...
            'o.total',
            'o.paid_total',
            (0, kysely_1.sql) `COALESCE(GROUP_CONCAT(DISTINCT pm.name ORDER BY pm.name SEPARATOR ', '), 'Sin método')`.as('payment_methods'),
            (0, kysely_1.sql) `MAX(d.created_at)`.as('delivered_at')
        ])
        ...
        const sessionPayments = await db
            ...
            'p.reference',
            'p.created_at',
            (0, kysely_1.sql) `pm.name`.as('payment_method_name')
        ])
            .where('p.created_at', '>=', active.opened_at)
